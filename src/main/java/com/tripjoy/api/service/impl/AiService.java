...
public class AiService implements IAiService {
    ...
    public Mono<AiFinalItineraryDto> modifyItinerary(AiModifyItineraryRequestDto request) {
        log.info(
                "Sending modify-itinerary request to AI Service with {} unwanted locations",
                request.getUnwantedLocations() != null
                        ? request.getUnwantedLocations().size()
                        : 0);
    ...
    public Mono<AiNotebookResponseDto> generateNotebook(AiGenerateNotebookRequestDto request) {
        log.info(
                "Sending generate-notebook request to AI Service for destination: {}. Payload: {}",
    ...
    public Mono<List<AiTripItemDto>> suggestLocation(AiSuggestLocationRequestDto request) {
        log.info(
                "Sending suggest-locations request to AI Service for unwanted location: {}",
                request.getUnwantedLocation() != null
                        ? request.getUnwantedLocation().getLocationName()
                        : "null");
