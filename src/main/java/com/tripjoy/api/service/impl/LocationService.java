...
public class LocationService implements ILocationService {
    ...
    public LocationResponse getOrCreateLocation(LocationCreateRequest request) {
        ...
        if (!nearbyLocations.isEmpty()) {
            ...
            log.info("Location already exists at similar coordinates: {} (<50m away)", existing.getName());
        ...
        log.info("Created new location: {} at ({}, {})", saved.getName(), saved.getLatitude(), saved.getLongitude());
    ...
    public LocationResponse updateLocation(UUID locationId, LocationCreateRequest request) {
        ...
        if (request.getOperationalStatus() != null
                && !request.getOperationalStatus().trim().isEmpty()) {
            location.setOperationalStatus(locationMapper.stringToOperationalStatus(request.getOperationalStatus()));
        }
    ...
    public List<LocationResponse> getNearbyLocations(
        ...
        log.info(
                "Finding nearby locations: lat={}, lng={}, radius={}m, categories={}, limit={}",
                latitude,
                longitude,
                radiusMeters,
                categories,
        ...
        if (maxResults > 100) maxResults = 100; // Max 100 results
        ...
        if (categories != null && !categories.isEmpty()) {
            nearbyLocations = nearbyLocations.stream()
                    .filter(location -> {
                        if (location.getPoiCategories() == null) return false;
                        return location.getPoiCategories().stream().anyMatch(categories::contains);
                    })
                    .limit(maxResults)
                    .toList();
        } else {
            nearbyLocations = nearbyLocations.stream().limit(maxResults).toList();
        }

        log.info("Found {} nearby locations", nearbyLocations.size());
        return nearbyLocations.stream().map(locationMapper::toResponse).toList();
    }
    ...
    public Page<LocationResponse> searchLocations(
        ...
        log.info(
                "Searching locations: query='{}', city='{}', district='{}', categories={}, page={}",
                query,
                city,
                district,
                categories,
        ...
        Page<Location> locations = locationRepository.searchLocations(query, city, district, pageable);
        ...
        if (categories != null && !categories.isEmpty()) {
            ...
            log.info(
                    "Found {} locations (filtered by categories from {})",
                    filtered.size(),
