...
public class ItineraryGenerationService implements IItineraryGenerationService {
        ...
        public ItineraryResponse initiateGeneration(GenerateItineraryRequest request, UUID userId) {
                ...
                Itinerary itinerary = Itinerary.builder()
                                ...
                                .budgetEstimate(
                                                request.getBudgetEstimate() != null
                                                                ? BigDecimal.valueOf(request.getBudgetEstimate())
                                                                : null)
        ...
        public void processGenerationAsync(UUID itineraryId, GenerateItineraryRequest request) {
                ...
                try {
                        ...
                        if (aiResponse == null || aiResponse.getTripItems() == null) {
                                log.error("AI Service returned empty or null trip items for itinerary ID: {}",
                                                itineraryId);
                        ...
                        resolveMissingPlaceIds(aiResponse.getTripItems(), request.getDestination(),
                        ...
                        List<TripItem> tripItems = aiResponse.getTripItems().stream().map(aiItem -> {
                                Location location = locationRepository.findByProviderId(aiItem.getPlaceId())
                                                .orElse(null);
        ...
        private AiTravelRequestDto mapToAiRequest(GenerateItineraryRequest req) {
                return AiTravelRequestDto.builder()
                                ...
                                .budget(req.getBudgetEstimate() != null
        ...
        Location location = Location.builder()
                ...
                if (dto.getLocation() != null && dto.getLocation().getLatitude() != null
                        ...
                        Point point = geometryFactory
                                        .createPoint(new Coordinate(dto.getLocation().getLongitude(),
        ...
        public ItineraryResponse modifyItinerary(UUID itineraryId, AiModifyItineraryRequest request) {
                ...
                List<AiTripItemDto> aiAllItems = currentItems.stream()
                                .map(item -> AiTripItemDto.builder()
                                                .startTime(item.getStartTime() != null
                                                                ? item.getStartTime().toString()
                                                                : null)
                                                .duration(item.getDuration())
                                                .note(item.getNote())
                                                .locationName(item.getLocation() != null
                                                                ? item.getLocation().getName()
                                                                : null)
                                                .placeId(item.getLocation() != null
                ...
                AiFinalItineraryDto aiItineraryDto = AiFinalItineraryDto.builder()
                                ...
                                .budgetEstimate(itinerary.getBudgetEstimate() != null
                                                ...
                                                : null)
                                .themes(itinerary.getThemes().stream().map(t -> t.getName())
                                                .collect(Collectors.toList()))
                                .destination(itinerary.getDestination() != null ? itinerary.getDestination().getName()
                                                : itinerary.getName().replaceFirst("(?i)^trip to ", "").trim())
                ...
                Double lat = referenceItem.getLocation() != null ? referenceItem.getLocation().getLatitude() : 0.0;
                Double lng = referenceItem.getLocation() != null ? referenceItem.getLocation().getLongitude() : 0.0;
                ...
                List<TripItem> replacementItems = aiResponse.getTripItems().stream()
                                .filter(aiItem -> aiItem.getPlaceId() != null
                                                && !keptPlaceIds.contains(aiItem.getPlaceId()))
                                .map(aiItem -> {
                                        ...
                                        return TripItem.builder()
                                                        ...
                                                        .startTime(aiItem.getStartTime() != null
                                                                        ? LocalDateTime.parse(aiItem.getStartTime(),
                                                                                        DateTimeFormatter.ISO_DATE_TIME)
                                                                        : null)
                                                        .build();
                                })
                ...
                log.info("Added {} replacement trip item(s) for itinerary ID: {}", replacementItems.size(),
                                itineraryId);
        ...
        public List<TripItemResponse> suggestLocation(UUID itineraryId, AiSuggestLocationRequest request) {
                log.info("Suggesting location replacement for itinerary ID: {}, unwanted place: {}",
                                itineraryId, request.getUnwantedPlaceId());
                ...
                AiTripItemDto unwantedDto = AiTripItemDto.builder()
                                .startTime(unwantedItem.getStartTime() != null
                ...
                return aiSuggestions.stream()
                                .map(ai -> TripItemResponse.builder()
                                                ...
                                                .startTime(ai.getStartTime() != null
                                                        ...
                                                        .map(resp -> {
                                                                if (resp != null && resp.getSuggestions() != null
                                                                        ...
                                                                        String resolvedPlaceId = resp.getSuggestions()
                                                                                        .get(0).getPlacePrediction()
                                                                                        .getPlaceId();
                                                                        item.setPlaceId(resolvedPlaceId);
                                                                        log.info("Resolved '{}' -> place_id: {}",
                                                                                        item.getLocationName(),
                                                                                        resolvedPlaceId);
                                                                } else {
                                                                        log.warn("Could not resolve '{}' on Google Maps.",
                                                                                        item.getLocationName());
                                                                }
                                                                return item;
                                                        })
        ...
        public AiFinalItineraryDto getLatestAiItineraryByGroupId(UUID groupId) {
                ...
                return AiFinalItineraryDto.builder()
                        .name(latest.getName())
                        .startDate(latest.getStartDate() != null ? latest.getStartDate().toLocalDate().toString() : null)
                        .endDate(latest.getEndDate() != null ? latest.getEndDate().toLocalDate().toString() : null)
                        .peopleQuantity(latest.getPeopleQuantity())
                        .budgetEstimate(latest.getBudgetEstimate() != null ? latest.getBudgetEstimate().longValue() : null)
                        .themes(latest.getThemes() != null ? latest.getThemes().stream().map(Theme::getName).collect(Collectors.toList()) : null)
                        .destination(latest.getDestination() != null ? latest.getDestination().getName() : null)
                        .tripItems(items.stream().map(item -> AiTripItemDto.builder()
