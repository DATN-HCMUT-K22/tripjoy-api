...
public class GroupService implements IGroupService {
    ...
    public GroupResponse createGroup(GroupRequest request, UUID ownerId) {
        // --- STEP 0: FETCH OWNER ---
        User owner = userRepository.findById(ownerId).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));
        ...
        if (group.getItineraries() == null) group.setItineraries(new java.util.HashSet<>());
        if (group.getConversations() == null) group.setConversations(new java.util.HashSet<>());
    ...
    public void leaveGroup(UUID groupId, UUID currentUserId) {
        ...
        GroupMember memberToLeave = groupMemberRepository
    ...
    public void transferLeadership(UUID groupId, TransferLeadershipRequest request, UUID currentUserId) {
        ...
        User newLeaderUser = userRepository
                .findById(request.getNewLeaderId())
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));

        GroupMember newLeader = groupMemberRepository
                .findByGroupAndUser(group, newLeaderUser)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_IN_GROUP));
