...
public class ChatMessageService implements IChatMessageService {
    ...
    public MessageCursorResponse getMessages(
        ...
        if (before != null) {
            ...
            messages = chatMessageRepository.findMessagesBefore(conversationId, beforeTime, pageable);
        } else if (after != null) {
            // Load newer messages
            LocalDateTime afterTime = LocalDateTime.parse(after);
            messages = chatMessageRepository.findMessagesAfter(conversationId, afterTime, pageable);
            Collections.reverse(messages); // Reverse to DESC order
        } else {
            // Initial load: latest messages
            messages = chatMessageRepository.findLatestMessages(conversationId, pageable);
        }
        ...
        if (!messageResponses.isEmpty()) {
            ...
            String lastTimestamp = messageResponses
                    .get(messageResponses.size() - 1)
                    .getCreatedAt()
                    .toString();
    ...
    public List<MessageSearchResponse> searchMessages(
        ...
        List<ChatMessage> results =
                chatMessageRepository.searchMessages(conversationId, sanitizedKeyword, pageSize, offset);
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MessageSearchResponse> searchMessagesGlobal(UUID currentUserId, String keyword, int page, int size) {
        ...
        List<ChatMessage> results =
                chatMessageRepository.searchMessagesGlobal(currentUserId, sanitizedKeyword, pageSize, offset);
