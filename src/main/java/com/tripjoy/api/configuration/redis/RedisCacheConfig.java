 ...
 *
 * <p>
 * <b>Key prefix:</b> {@code tripjoy:} is prepended to all cache names to avoid
 * collisions with Socket.IO on Redis DB 1.
 *
 * <p>
 * <b>IMPORTANT:</b> All entries MUST have TTL for the volatile-lru eviction
 * policy.
 */
@Configuration
@EnableCaching
...
public class RedisCacheConfig {
    ...
    /**
     * Autocomplete suggestions: hybrid DB + Google. Short TTL (10min) because POIs
     * change frequently.
     */
    ...
    /**
     * Public profile: visible to any authenticated user, no sensitive fields. TTL
     * 12h.
     */
    ...
    // Itinerary
    public static final String CACHE_ITINERARIES_BY_USER = "itineraries:user";
    ...
    public RedisCacheManager cacheManager(RedisConnectionFactory connectionFactory) {
        // 1. Setup a dedicated ObjectMapper for Redis with standard Enterprise settings
        ...
        // Location
        cacheConfigs.put(CACHE_LOCATION_BY_ID, defaults.entryTtl(TTL_LOCATION_ID));
        cacheConfigs.put(CACHE_LOCATION_BY_PROVIDER, defaults.entryTtl(TTL_LOCATION_PROVIDER));
        cacheConfigs.put(CACHE_LOCATION_ADMIN, defaults.entryTtl(TTL_LOCATION_ADMIN));
        cacheConfigs.put(CACHE_LOCATION_AUTOCOMPLETE, defaults.entryTtl(TTL_LOCATION_AUTOCOMPLETE));

        // Auth — JWT token blacklist
        cacheConfigs.put(CACHE_INVALIDATED_TOKEN, defaults.entryTtl(TTL_INVALIDATED_TOKEN));

        // RBAC config data
        cacheConfigs.put(CACHE_PERMISSION_ALL, defaults.entryTtl(TTL_PERMISSION_ALL));
        cacheConfigs.put(CACHE_ROLE_ALL, defaults.entryTtl(TTL_ROLE_ALL));
        ...
        cacheConfigs.put(CACHE_GROUP_MEMBERS, defaults.entryTtl(TTL_GROUP_MEMBERS));

        // Itinerary
        cacheConfigs.put(CACHE_ITINERARIES_BY_USER, defaults.entryTtl(Duration.ofMinutes(10)));
