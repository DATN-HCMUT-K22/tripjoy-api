...
public class MediaController {
    ...
    @Operation(summary = "Upload an image (server-side)", description = """
    ...
    public ApiResponse<MediaUploadResponse> uploadImage(
            @Parameter(description = "Image file (JPG/PNG/WEBP/GIF/HEIC, max 10MB)", required = true, content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)) @RequestPart("file") MultipartFile file,
            @Parameter(description = "Target folder on Cloudinary (e.g. 'tripjoy/avatars/users')") @RequestParam(value = "folder", defaultValue = "tripjoy/misc") String folder) {
    ...
    @Operation(summary = "Upload a video (server-side)", description = """
    ...
    public ApiResponse<MediaUploadResponse> uploadVideo(
            @Parameter(description = "Video file (MP4/MOV/AVI/WEBM, max 50MB)", required = true, content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)) @RequestPart("file") MultipartFile file,
            @Parameter(description = "Target folder on Cloudinary (e.g. 'tripjoy/messages/conversationId')") @RequestParam(value = "folder", defaultValue = "tripjoy/misc") String folder) {
    ...
    @Operation(summary = "Delete a media file", description = "Permanently removes a file from Cloudinary by its publicId.")
    @DeleteMapping(Endpoint.Media.DELETE)
    public ApiResponse<Void> deleteMedia(
            @Parameter(description = "Cloudinary public_id (e.g. 'tripjoy/avatars/users/abc123')", required = true) @RequestParam("publicId") String publicId,
            @Parameter(description = "Resource type: 'image' or 'video'") @RequestParam(value = "resourceType", defaultValue = "image") String resourceType) {
    ...
    @Operation(summary = "Generate signed upload credentials (for React Native direct upload)", description = """
    ...
    public ApiResponse<Map<String, Object>> generateUploadSignature(
            @Parameter(description = "Target folder (e.g. 'tripjoy/posts')") @RequestParam(value = "folder", defaultValue = "tripjoy/misc") String folder,
            @Parameter(description = "Optional Cloudinary upload preset name") @RequestParam(value = "uploadPreset", required = false) String uploadPreset) {
